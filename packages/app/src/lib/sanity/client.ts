...
export function urlFor(source: Partial<SanityAsset>) {
	if (source) return imageBuilder.image(source);
}

export function getImageDimensions(image: Pick<SanityImageObject, 'asset'>) {
	if (!image?.asset?._ref) {
	  return
	}
...
export function getImageProps({
  ...
  return {
    ...
    width: retinaSizes[0],
    height: retinaSizes[0] / (imageDimensions?.aspectRatio || 1),
  };
}